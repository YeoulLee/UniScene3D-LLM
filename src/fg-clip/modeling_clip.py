...
class FourierXYZ(nn.Module):
    """
    xyz: (B,N,3) -> (B,N, 6*K) using sin/cos at K frequencies per axis.
    """
    def __init__(self, num_bands: int = 10, max_freq: float = 10.0):
        super().__init__()
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        # xyz: (B,N,3)
        x = xyz.unsqueeze(-1) * self.freqs.view(1, 1, 1, -1) * math.pi  # (B,N,3,K)
        feat = torch.cat([torch.sin(x), torch.cos(x)], dim=-1)          # (B,N,3,2K)
        return feat.flatten(2, 3)                                       # (B,N,6K)


# ----------------------------
# Patchify point map by average pooling
# ----------------------------

def patchify_xyz(point_map: torch.Tensor, patch_size: int) -> torch.Tensor:
    """
    point_map: (B,3,H,W)
    returns:   (B,N,3) where N=(H/ps)*(W/ps)
    """
    ps = patch_size
    xyz = F.avg_pool2d(point_map, kernel_size=ps, stride=ps)  # (B,3,gh,gw)
    xyz_tokens = xyz.flatten(2).transpose(1, 2).contiguous()  # (B,N,3)
    return xyz_tokens
...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def geo_forward(self, pm_values: torch.FloatTensor) -> torch.Tensor:
        target_dtype = self.patch_embedding.weight.dtype
        geo_pos = self.geo_embedding(pm_values.to(dtype=target_dtype))
        return geo_pos.flatten(2).transpose(1, 2)
    ...
    def forward(
        ...
        if pixel_values.shape[1] == 3:
            ...
            batch_size = pixel_values.shape[0]
            pixel_values, pm_values = pixel_values[:,:3,:,:], pixel_values[:,3:,:,:]
            _, _, height, width = pixel_values.shape

            if not use_patch_embedding and not use_geo_embedding:
                raise ValueError("At least one of use_patch_embedding or use_geo_embedding must be True.")

            target_dtype = self.patch_embedding.weight.dtype
            patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
            patch_embeds = patch_embeds.flatten(2).transpose(1, 2)

            if not use_patch_embedding:
                patch_embeds = torch.zeros_like(patch_embeds)

            geo_pos = self.geo_forward(pm_values)

            if not use_geo_embedding:
                geo_pos = torch.zeros_like(geo_pos)
            patch_embeds = patch_embeds + geo_pos
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        # print(positional_embedding_pre.shape)
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, CLIPTextEmbeddings):
            ...
            nn.init.normal_(module.geo_embedding.weight, std=module.config.initializer_range * factor)
            # nn.init.normal_(module.geo_embedding.mlp[-1].weight, std=module.config.initializer_range * factor)
